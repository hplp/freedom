...
class ZCU102MIGIODDR(depth : BigInt) extends GenericParameterizedBundle(depth) {
  require((depth<=0x20000000L),"ZCU102MIGIODDR supports upto 512MB depth configuraton")
...
{
  require((depth<=0x20000000L),"zcu102mig supports upto 512MB depth configuraton")

  val io = new ZCU102MIGIODDR(depth) with ZCU102MIGIOClocksReset {
    //slave interface write address ports
    val c0_ddr4_s_axi_awid            = Bits(INPUT,4)
    val c0_ddr4_s_axi_awaddr          = Bits(INPUT,29)
    ...
    val c0_ddr4_s_axi_araddr          = Bits(INPUT,29)
    ...
    """ 
      create_ip -vendor xilinx.com -library ip -version 2.1 -name ddr4 -module_name zcu102mig -dir $ipdir -force
