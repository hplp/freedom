...
package sifive.fpgashells.ip.xilinx.vcu110mig
...
class VCU110MIGIODDR(depth : BigInt) extends GenericParameterizedBundle(depth) {
  require((depth<=0x80000000L),"VCU110MIGIODDR supports upto 2GB depth configuraton")
...
trait VCU110MIGIOClocksReset extends Bundle {
  //inputs
...
class vcu110mig(depth : BigInt)(implicit val p:Parameters) extends BlackBox
{
  require((depth<=0x80000000L),"vcu110mig supports upto 2GB depth configuraton")

  val io = new VCU110MIGIODDR(depth) with VCU110MIGIOClocksReset {
  ...
  ElaborationArtefacts.add(
    "vcu110mig.vivado.tcl",
    """ 
      create_ip -vendor xilinx.com -library ip -version 2.1 -name ddr4 -module_name vcu110mig -dir $ipdir -force
