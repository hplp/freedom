...
import sifive.fpgashells.ip.xilinx.ua280mig.{UA280MIGIOClocksReset, UA280MIGIODDR, ua280mig}

case class XilinxUA280MIGParams(
  address : Seq[AddressSet]
)

class XilinxUA280MIGPads(depth : BigInt) extends UA280MIGIODDR(depth) {
  def this(c : XilinxUA280MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxUA280MIGIO(depth : BigInt) extends UA280MIGIODDR(depth) with UA280MIGIOClocksReset

class XilinxUA280MIGIsland(c : XilinxUA280MIGParams)(implicit p: Parameters) extends LazyModule with HasCrossing {
  ...
  // Tranfer size funtion specify the min and the max transfer size
  ...
  lazy val module = new LazyModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxUA280MIGIO(depth)
    })

    //MIG black box instantiation
    val blackbox = Module(new ua280mig(depth))
    ...
    blackbox.io.c0_ddr4_s_axi_ctrl_awaddr := UInt("h00000000")
    blackbox.io.c0_ddr4_s_axi_ctrl_wvalid := false.B
    blackbox.io.c0_ddr4_s_axi_ctrl_wdata := UInt("h00000000")
    blackbox.io.c0_ddr4_s_axi_ctrl_bready := false.B
    blackbox.io.c0_ddr4_s_axi_ctrl_arvalid := false.B
    blackbox.io.c0_ddr4_s_axi_ctrl_araddr := UInt("h00000000")
    blackbox.io.c0_ddr4_s_axi_ctrl_rready := false.B
...
class XilinxUA280MIG(c : XilinxUA280MIGParams)(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxUA280MIGIsland(c))
