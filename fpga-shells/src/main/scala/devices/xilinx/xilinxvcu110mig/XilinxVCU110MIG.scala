...
import sifive.fpgashells.ip.xilinx.vcu110mig.{VCU110MIGIOClocksReset, VCU110MIGIODDR, vcu110mig}

case class XilinxVCU110MIGParams(
  address : Seq[AddressSet]
)

class XilinxVCU110MIGPads(depth : BigInt) extends VCU110MIGIODDR(depth) {
  def this(c : XilinxVCU110MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxVCU110MIGIO(depth : BigInt) extends VCU110MIGIODDR(depth) with VCU110MIGIOClocksReset

class XilinxVCU110MIGIsland(c : XilinxVCU110MIGParams)(implicit p: Parameters) extends LazyModule with HasCrossing {
  ...
  require((depth<=0x80000000L),"vcu110mig supports upto 2GB depth configuraton")
  ...
  lazy val module = new LazyModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxVCU110MIGIO(depth)
    })

    //MIG black box instantiation
    val blackbox = Module(new vcu110mig(depth))
...
class XilinxVCU110MIG(c : XilinxVCU110MIGParams)(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxVCU110MIGIsland(c))
